...
fs.readFile(file, 'utf8', (error, fileData) => {
  ...
  let stats;
  try {
    stats = fs.statSync(options.lang);
  } catch(e) {
  ...
  if (stats && stats.isFile()) {
    content = fs.readFileSync(options.lang, 'utf8');
    contentObject = JSON.parse(content);
    translations = _.extend({}, contentObject, translations);
    // replace the file and tell the user.
    logme.info(options.lang + ' exists so we will merge it\'s content with the new one.');
  }
